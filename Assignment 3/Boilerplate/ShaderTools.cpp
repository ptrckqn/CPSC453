...
GLuint ShaderTools::InitializeShaders(int quadBez) {
	...
	std::string tcsSourceTwo = LoadSource("shaders/tessControlTwo.glsl");
	...
	GLuint tcst = CompileShader(GL_TESS_CONTROL_SHADER, tcsSourceTwo);
	...
	if (quadBez == 0){
		...
		// link shader program
		program = LinkProgram(vertex, fragment, tcst, tes);
	}
